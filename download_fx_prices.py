...
from pipeline_utils import BASE_DIR, atomic_write_dataframe_csv, configure_yfinance_cache, run_cli

NEWS_PATH = BASE_DIR / "processed_news.jsonl"
OUTPUT_PATH = BASE_DIR / "fx_prices.csv"
...
def main():
    ...
    if prices.empty or prices.dropna(how="all").empty:
        raise RuntimeError("모든 환율 다운로드가 실패해 기존 fx_prices.csv를 보존합니다.")
    atomic_write_dataframe_csv(OUTPUT_PATH, prices)
