...
def collect_fred_indicators() -> dict:
    ...
    for name, series_id in FRED_SERIES.items():
        try:
            ...
            print(f"[FRED] {name} ({series_id}) 실패: {safe_error(e, [FRED_API_KEY])}")
            result[name] = []
...
def collect_ecos_indicators() -> dict:
    ...
    for name, cfg in ECOS_TARGETS.items():
        ...
        try:
            ...
            print(f"[ECOS] {name} 실패: {safe_error(e, [ECOS_API_KEY])}")
            result[name] = []
        time.sleep(0.2)
    return result
...
def collect_foreign_netflow() -> list[dict]:
    ...
    try:
        ...
        print(f"[pykrx] 수집 실패: {safe_error(e)}")
        return []
...
def collect_google_trends(keywords: list[str] = None) -> list[dict]:
    ...
    try:
        ...
        print(f"[pytrends] 수집 실패 (Google Trends는 요청이 잦으면 잘 막힘): {safe_error(e)}")
        return []
...
def main():
    ...
    atomic_write_json(OUTPUT_PATH, macro_data)
