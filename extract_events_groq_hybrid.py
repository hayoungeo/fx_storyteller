...
INPUT_PATH = BASE_DIR / "collected_news.jsonl"
OUTPUT_PATH = BASE_DIR / "processed_news.jsonl"
...
def call_groq(api_key: str, article: dict, hinted_category: str,
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(
                f"[{article.get('id')}] 요청 오류 (시도 {attempt+1}/{max_retries}): "
...
def process_all() -> None:
    ...
    for i, (article, hinted_category) in enumerate(candidates, 1):
        ...
        if extracted.pop("_groq_failed", False):
            failed_count += 1
    ...
    if candidates and failed_count == len(candidates):
        raise RuntimeError(
            "모든 Groq 호출이 실패해 기존 processed_news.jsonl을 보존합니다."
        )
