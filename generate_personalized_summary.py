...
WEIGHTS_PATH = BASE_DIR / "impact_weights.json"
ASSETS_PATH = BASE_DIR / "user_assets.json"
OUTPUT_PATH = BASE_DIR / "personalized_summary.json"
...
def call_groq(
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(
                f"[{asset['name']}] 요청 오류 (시도 {attempt+1}/{max_retries}): "
                f"{safe_error(e, [api_key])}"
            )
...
def process_all():
    ...
    for asset in assets:
        ...
        if not matched and not volatility_context:
            ...
            if summary_text is None:
                llm_failures += 1
                summary_text = "(생성 실패 - Groq 응답을 받지 못했습니다)"
    ...
    if llm_attempts and llm_failures == llm_attempts:
        raise RuntimeError(
            "모든 개인화 요약 생성이 실패해 기존 personalized_summary.json을 보존합니다."
        )
