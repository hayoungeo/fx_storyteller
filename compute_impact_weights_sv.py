...
--------------------------------------------------------------------------------
카테고리별 뉴스가 있는 날의 평균 변동성을 baseline과 비교해서 가중치를 산출한다.
...
from pipeline_utils import BASE_DIR, atomic_write_json, run_cli
from volatility_context import normalize_pair

NEWS_PATH = BASE_DIR / "processed_news.jsonl"
SV_PATH = BASE_DIR / "sv_volatility.json"
OUTPUT_PATH = BASE_DIR / "impact_weights.json"
...
def load_sv_volatility(path: Path) -> dict[str, dict[str, float]]:
    ...
    for pair, records in raw.items():
        ...
        if isinstance(records, dict):
            records = [records]
        result[normalize_pair(pair)] = {r["date"]: r["sigma"] for r in records}
...
def main():
    ...
    if not any(info["sample_size"] > 0 for info in ratios.values()):
        raise RuntimeError("뉴스 날짜와 SV 데이터가 겹치지 않아 기존 가중치 파일을 보존합니다.")
